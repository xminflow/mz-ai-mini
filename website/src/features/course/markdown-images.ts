// 把课程 Markdown 里的图片引用解析成站点可访问的绝对路径 /courses/<chapterId>/...
//
...
// public/ 下的实际文件；统一改写成 /courses/<chapterId>/... 后，浏览器即可正常请求到图片。
export function resolveCourseImages(content: string, chapterId: string): string {
  const base = `/courses/${chapterId}`
