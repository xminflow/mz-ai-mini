...
6. 在 `meta.ts` 里填 `sceneId`（取值必须是 `taxonomy.ts` 里已有的场景 id）
   与 `platform`（`pc` / `miniprogram` / `mobile`）
...
- **分类维度只有一个：客户要做的是什么**（企业官网、进销存、客户管理……）。
  载体不进分类，它是模板自己的属性 `platform`（`pc` / `miniprogram` / `mobile`，见 `types.ts`）。
  同一件事在不同端上做，对客户来说仍是同一件事，拆成两个分类只会让「电商商城」
  这类条目在目录里出现两次。卡片眉标是「业务形态 · 端」两个维度并列
- `platform` 是单值不是数组：PC 版与小程序版当成两套模板分开做。一套模板只有一份页面代码、
