...
- `basePath` 传给页面组件的是**端根路径**（`/templates/meridian/site`），不是模板根路径
- 预览页多一层端切换，只在多端时出现。**切端一律回落到该端首页**——
  两个端的页面 slug 没有对应关系，带着当前 slug 切过去多半直接 404
- 多端时预览窗口的示意域名用端 id 做子域（`admin.meridian.example.com`），
