...
const TITLE_BADGES = [
  { label: '创业公司 CTO', color: '#57beff', bg: 'rgba(0,153,255,0.10)', border: 'rgba(87,190,255,0.45)' },
  { label: '一线工程师', color: '#01aef0', bg: 'rgba(1,174,240,0.10)', border: 'rgba(1,174,240,0.45)' },
  { label: 'AI 教学者', color: '#ff52b7', bg: 'rgba(212,38,114,0.10)', border: 'rgba(255,82,183,0.45)' },
]

export function CtoSection() {
  return (
    <section className="relative mx-auto w-full max-w-2xl px-4 pt-10 sm:px-6 sm:pt-14">
      <Reveal>
        <div
          className="relative overflow-hidden rounded-md p-6 backdrop-blur-xl sm:p-8"
          style={{
            background: 'linear-gradient(150deg, rgba(0,153,255,0.16) 0%, rgba(0,153,255,0.05) 100%)',
        ...
        >
          <span
            ...
            style={{ background: 'radial-gradient(circle, rgba(87,190,255,0.5) 0%, transparent 65%)' }}
          />
          <span
            aria-hidden
            className="pointer-events-none absolute -bottom-16 -left-16 h-44 w-44 rounded-full opacity-60"
            style={{ background: 'radial-gradient(circle, rgba(212,38,114,0.4) 0%, transparent 65%)' }}
          />
          ...
          <div className="relative flex flex-col gap-5">
            <div className="flex items-center gap-4">
              <img
                ...
                style={{
                  ...
                  boxShadow: '0 12px 28px -8px rgba(0,153,255,0.65), inset 0 0 0 1px rgba(255,255,255,0.2)',
                }}
              />
            ...
            <div className="flex flex-col gap-3 border-t pt-4" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
              {INSTRUCTOR_CREDENTIALS.map((cred) => {
                const t = THEMES[cred.theme]
                return (
                  <div key={cred.label} className="flex flex-col gap-1 border-l-2 pl-3" style={{ borderColor: t.hex }}>
                    <div className="flex items-baseline gap-2">
                      <span
                        className="font-serif-zh text-[15px] font-bold leading-none"
                        style={{ color: t.hex, textShadow: `0 0 14px ${t.hex}55` }}
                      >
                        {cred.metric}
                      </span>
                    ...
                    <span className="text-[11.5px] leading-[1.65] text-ink-soft">{cred.detail}</span>
                  </div>
                )
              })}
            </div>
            ...
            <div className="flex items-center gap-2">
              <span
                ...
                style={{ background: '#57beff', boxShadow: '0 0 6px rgba(87,190,255,0.8)' }}
              />
              <span className="text-[12.5px] font-medium text-ink sm:text-[13px]">
                每个部门，我都亲自参与关键决策，不做甩手掌柜
              </span>
            </div>
          </div>
        </div>
