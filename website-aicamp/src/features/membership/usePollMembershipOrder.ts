...
type PollState =
  ...
  | { status: 'paid'; error: null; result: CampOrderStatusResponse; orderNo: string }
...
export function usePollMembershipOrder({ orderNo, qrExpiresAt, enabled }: Options): PollState {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        const result = await getCampMembershipOrder(orderNo)
        consecutiveErrors = 0
        ...
        consecutiveErrors += 1
        console.error({ scope: 'camp-membership', endpoint: 'order_status', error })
