...
const buildTableCell = (text, columnIndex, align, prefix) => ({
  id: `${prefix}-cell-${columnIndex}`,
  align,
  inlines: parseInlineSegments(text),
});

const parseTableBlock = (lines, startIndex, blockIndex) => {
...
const parseInlineSegments = (text) => {
  ...
  while (matched) {
    ...
    } else if (matched[3] !== undefined || matched[4] !== undefined) {
      pushInlineSegment(segments, {
        type: "strong",
        text: matched[3] || matched[4],
      });
    } else if (matched[5] !== undefined) {
      pushInlineSegment(segments, {
        type: "inline-code",
        text: matched[5],
      });
    } else if (matched[6] !== undefined || matched[7] !== undefined) {
      pushInlineSegment(segments, {
        type: "emphasis",
        text: matched[6] || matched[7],
      });
    }
...
const parseMarkdownToBlocks = (markdown) => {
  ...
  while (lineIndex < lines.length) {
    ...
    const table = parseTableBlock(lines, lineIndex, blocks.length);
    if (table) {
    ...
    if (quoteLine !== null) {
      ...
      blocks.push({
        ...
        type: "quote",
        inlines: parseInlineSegments(quoteLines.join(" ").trim()),
      });
      continue;
    }
    ...
    if (unorderedListLine !== null) {
      ...
      while (lineIndex < lines.length) {
        ...
        items.push({
          ...
          marker: "•",
          inlines: parseInlineSegments(listItemText),
        });
        lineIndex += 1;
      }
    ...
    if (orderedListLine) {
      ...
      while (lineIndex < lines.length) {
        ...
        items.push({
          ...
          inlines: parseInlineSegments(listItem.text),
        });
        lineIndex += 1;
      }
    ...
    blocks.push({
      ...
      inlines: containsFencedCodeSyntax(paragraphLines)
        ...
        : parseInlineSegments(paragraphText),
    });
  }
