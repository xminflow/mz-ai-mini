...
export interface BuildHotMaterialReportPromptInput {
  dataRoot: string;
  guideRoot: string;
  runId: string;
  platform: Platform;
...
export function buildHotMaterialReportPrompt(
  input: BuildHotMaterialReportPromptInput,
): string {
  const platformLabel = input.platform === "xiaohongshu" ? "小红书" : "抖音";
  return [
    "Use the douyin-hot-material-report skill to generate the final hot material analysis report.",
    "",
    "Parameters:",
    ...
    "You may read Markdown files under guide_root to connect the analysis to 百万粉博主流量实战.",
