...
import {
  hotMaterialAnalyzeResultSchema,
  hotMaterialCreateResultSchema,
  hotMaterialDeleteResultSchema,
  hotMaterialGetReportResultSchema,
  hotMaterialListResultSchema,
...
const QUERY_KEY = ["hot-material-analysis"] as const;
const ANALYZE_MUTATION_KEY = ["hot-material-analysis", "analyze"] as const;
const SELECTED_ID_STORAGE_KEY = "hot-material-analysis:selected-id";
...
function statusLabel(status: HotMaterialAnalysis["status"]): string {
  switch (status) {
    case "pending":
      return "待采集";
    case "captured":
...
function statusVariant(
  status: HotMaterialAnalysis["status"],
...
function platformLabel(platform: HotMaterialAnalysis["platform"]): string {
...
function coverFramePath(item: HotMaterialAnalysis): string | null {
  return item.frames.find((frame) => frame.kind === "cover")?.path ?? item.frames[0]?.path ?? null;
}

function useHotMaterials() {
  return useQuery({
    queryKey: QUERY_KEY,
    queryFn: async () => {
      const raw = await window.api.hotMaterialAnalysis.list();
      const parsed = hotMaterialListResultSchema.safeParse(raw);
      if (!parsed.success) {
        return {
          schema_version: "1" as const,
          ok: false as const,
          error: { code: "INTERNAL" as const, message: "hot-material:list 响应未通过 Zod 校验" },
        };
      }
      return parsed.data;
    },
...
function useHotMaterialReport(id: string | null) {
  return useQuery<HotMaterialGetReportResult>({
    queryKey: ["hot-material-report", id],
    enabled: id !== null,
    queryFn: async () => {
      if (id === null) {
        return {
      ...
      const raw = await window.api.hotMaterialAnalysis.getReport({ id });
      const parsed = hotMaterialGetReportResultSchema.safeParse(raw);
      if (!parsed.success) {
        return {
          schema_version: "1",
          ok: false,
          error: { code: "INTERNAL", message: "hot-material:get-report 响应未通过 Zod 校验" },
        };
      }
      return parsed.data;
    },
...
export function HotAnalysisPage(): JSX.Element {
  ...
  useEffect(() => {
    const id = window.api.hotMaterialAnalysis.onEvent(() => {
      void qc.invalidateQueries({ queryKey: QUERY_KEY });
      if (selected?.id !== undefined) {
        void qc.invalidateQueries({ queryKey: ["hot-material-report", selected.id] });
      }
    });
    return () => window.api.hotMaterialAnalysis.offEvent(id);
  }, [qc, selected?.id]);

  const analyzeMut = useMutation({
    mutationKey: ANALYZE_MUTATION_KEY,
    mutationFn: async (input: { id: string }) => {
      const raw = await window.api.hotMaterialAnalysis.analyze(input);
      const parsed = hotMaterialAnalyzeResultSchema.safeParse(raw);
      if (!parsed.success) {
        return {
          schema_version: "1" as const,
          ok: false as const,
          error: { code: "INTERNAL" as const, message: "hot-material:analyze 响应未通过 Zod 校验" },
        };
      }
      return parsed.data;
    },
    onSettled: (_data, _error, vars) => {
      void qc.invalidateQueries({ queryKey: QUERY_KEY });
      void qc.invalidateQueries({ queryKey: ["hot-material-report", vars.id] });
    },
    onSuccess: (result) => {
  ...
  const createMut = useMutation({
    mutationFn: async (input: { share_url: string }) => {
      const raw = await window.api.hotMaterialAnalysis.create(input);
      const parsed = hotMaterialCreateResultSchema.safeParse(raw);
      if (!parsed.success) {
        return {
          schema_version: "1" as const,
          ok: false as const,
          error: { code: "INTERNAL" as const, message: "hot-material:create 响应未通过 Zod 校验" },
        };
      }
      return parsed.data;
    },
  ...
  const deleteMut = useMutation({
    mutationFn: async (input: { id: string }) => {
      const raw = await window.api.hotMaterialAnalysis.delete(input);
      const parsed = hotMaterialDeleteResultSchema.safeParse(raw);
      if (!parsed.success) {
        return {
          schema_version: "1" as const,
          ok: false as const,
          error: { code: "INTERNAL" as const, message: "hot-material:delete 响应未通过 Zod 校验" },
        };
      }
      return parsed.data;
    },
    ...
    <div className="flex h-[calc(100vh-104px)] min-h-0 flex-col gap-5">
      <header className="page-header">
        <div className="min-w-0">
          <h1 className="page-title">爆款分析</h1>
          <p className="page-description">
            添加抖音或小红书视频链接，自动采集、提取文案、分析爆款机制。
          </p>
        </div>
      ...
      <div className="grid min-h-0 flex-1 gap-4 overflow-hidden lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="surface-card flex min-h-0 flex-col overflow-hidden">
          <div className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-4">
            ...
            <h2 className="text-sm font-semibold">素材卡片</h2>
              ...
              <div className="flex-1 overflow-y-auto p-5">
                ...
                {reportQuery.isLoading || isAnalyzingSelected ? (
                  <div className="flex min-h-52 items-center justify-center text-sm text-muted-foreground">
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    {isAnalyzingSelected ? "正在处理素材并生成报告…" : "加载报告…"}
                  </div>
                ) : reportQuery.data?.ok ? (
                  ...
                  <div className="flex min-h-52 flex-col items-center justify-center gap-3 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {reportQuery.data?.error.message ?? selected.last_error ?? "尚未生成报告"}
                    </p>
                    <Button
                      onClick={() => analyzeMut.mutate({ id: selected.id })}
                      disabled={isAnalyzingAny}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>添加爆款分析</DialogTitle>
            <DialogDescription>
              粘贴抖音或小红书视频分享链接或整段分享文案，系统会先采集基础信息，再提取文案和画面素材生成分析报告。
            </DialogDescription>
          </DialogHeader>
          <FormField
